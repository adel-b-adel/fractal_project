...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/10/20 17:28:34 by rfrankly          #+#    #+#             */
/*   Updated: 2019/10/20 20:24:06 by rfrankly         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_celtic_mandelbrot(t_all *all)
{
	...
	while (y < WIN_HEIGHT)
	{
		...
		while (x < WIN_WIDTH)
		{
			all->pixels[x + y * WIN_WIDTH] = check_celt_mbr_point(all);
			x++;
...
int		check_celt_mbr_point(t_all *all)
{
	...
	{
		tmp_z_re = z.re;
		z.re = all->fract.c.re + fabsl(z.re * z.re - z.im * z.im);
